...
#                     )
    ...
    def __read_plmc_v1(self, filename, precision, alphabet=None):
        ...
        with open(filename, "rb") as f:
            ...
            # target sequence and index mapping, again ensure unicode
#             model_name = os.path.basename(filename) #get model filename
#             print(model_name)
#             filenames = glob.glob("/home/as974/ada/mutations/mutational_data/alignments/*.a2m") #get list of all alignments
#             name = model_name.split('.model')[0] #get alignment name
#             remove_lower = lambda text: re.sub('[a-z]', '', text) #function to get rid of non-sig alignment in target seq
            
#             f = [s for s in filenames if name in s][0] # match model with alignment name
            
#             with open(f,'r') as file:
#                 # find target sequence from MSA (first alignment)
#                 line = file.readlines()[1][:-1]
# #               line_no_lc = remove_lower(line)
#                 line_no_lc = line.upper()
#                 ts = np.asarray(list(line_no_lc),dtype='U1')
